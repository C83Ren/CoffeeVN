# game/r1_famres.rpy:23
translate simplified_chinese r1_famres_e3d45387:

    # "We walked to a nearby family restaurant."
    "两人一起去了附近的家庭餐厅。"

# game/r1_famres.rpy:27
translate simplified_chinese r1_famres_e550cdd7:

    # "We took our seats and looked at the menu."
    "店员" "「欢迎光临」"
    "找了个位子坐了下来，然后打开菜单。"

# game/r1_famres.rpy:29
translate simplified_chinese r1_famres_a70c1730:

    # p "You can get whatever you want, Shiraishi."
    p "「随便你选哦，小白石」"

# game/r1_famres.rpy:33
translate simplified_chinese r1_famres_0d9c0b87:

    # s "Really? Can I order more than one?"
    s "「真的？可以选不止一种吗？」"

# game/r1_famres.rpy:35
translate simplified_chinese r1_famres_04fa07a2:

    # p "You sure are hungry! Sure, go right ahead, order whatever you want~"
    p "「肚子饿了呢！没关系，随便你点什么都可以～」"

# game/r1_famres.rpy:39
translate simplified_chinese r1_famres_d89e56fe:

    # "Shiraishi flipped through the menu focusedly, choosing what to order."
    "白石非常专心地看着菜单，似乎决定好了要点什么。"

# game/r1_famres.rpy:41
translate simplified_chinese r1_famres_3d67b097:

    # p "Done choosing?"
    p "「决定好了吗？」"

# game/r1_famres.rpy:43
translate simplified_chinese r1_famres_d207e06f:

    # s "Yes!"
    s "「嗯！」"

# game/r1_famres.rpy:47
translate simplified_chinese r1_famres_06a71942:

    # p "Let’s call the waitress then."
    p "「那[player_name!t]去叫店员了哦」"

# game/r1_famres.rpy:49
translate simplified_chinese r1_famres_0968cd71:

    # "Waitress" "Are you ready to order?"
    "店员" "「决定好要点什么了吗？」"

# game/r1_famres.rpy:51
translate simplified_chinese r1_famres_f3e2da6e:

    # p "I’d like the carbonara pasta and fries. What about you, Shiraishi?"
    p "「[player_name!t]要培根蛋酱意面和薯条。小白石要点什么呢？」"

# game/r1_famres.rpy:55
translate simplified_chinese r1_famres_c736f63b:

    # s "Shiraishi wants omurice, beef bowl, and hamburger steak!"
    s "「白石要蛋包饭、牛肉饭和汉堡肉！」"

# game/r1_famres.rpy:57
translate simplified_chinese r1_famres_c8ea8a4a:

    # p "..."
    p "「…」"

# game/r1_famres.rpy:59
translate simplified_chinese r1_famres_aca929d8:

    # "Waitress" "Carbonara pasta, fries, omurice, beef bowl, and hamburger steak, right? Thank you for your order; please wait a bit."
    "店员" "「是培根蛋面和薯条，蛋包饭、牛肉盖饭和汉堡肉吧？感谢两位的惠顾，请稍作等待」"

# game/r1_famres.rpy:61
translate simplified_chinese r1_famres_07a81442:

    # "The waitress left."
    "店员离开了。"

# game/r1_famres.rpy:63
translate simplified_chinese r1_famres_c8ea8a4a_1:

    # p "..."
    p "「…」"

# game/r1_famres.rpy:65
translate simplified_chinese r1_famres_ba3a7a1e:

    # p "Ehhhhhhhh?!! You’re going to eat all that, Shiraishi??!"
    p "「诶——？！那么多的量，小白石要一个人吃完吗！？」"

# game/r1_famres.rpy:67
translate simplified_chinese r1_famres_016e969e:

    # "Shiraishi just replied with a bright smile."
    "白石只给[player_name!t]回了个笑容。"

# game/r1_famres.rpy:69
translate simplified_chinese r1_famres_39f79847:

    # "Not long after, the food came."
    "没过多久，点的菜就端上来了。"

# game/r1_famres.rpy:71
translate simplified_chinese r1_famres_34f34641:

    # p "As expected... this is really a lot..."
    p "「这个量…和预想的一样多…」"

# game/r1_famres.rpy:75
translate simplified_chinese r1_famres_62e97dc8:

    # s "Hitona onee-chan~"
    s "「[player_name!t]姐姐～」"

# game/r1_famres.rpy:77
translate simplified_chinese r1_famres_58ee35ab:

    # p "Hmmm?"
    p "「嗯？」"

# game/r1_famres.rpy:79
translate simplified_chinese r1_famres_39f5ca62:

    # s "Can Hitona onee-chan eat one of these?"
    s "「[player_name!t]姐姐能帮我吃一道菜吗？」"

# game/r1_famres.rpy:81
translate simplified_chinese r1_famres_c8ea8a4a_2:

    # p "..."
    p "「…」"

# game/r1_famres.rpy:83
translate simplified_chinese r1_famres_c8ea8a4a_3:

    # p "..."
    p "「…」"

# game/r1_famres.rpy:85
translate simplified_chinese r1_famres_ace7ef7e:

    # p "EHHHHH!?"
    p "「诶——！？」"

# game/r1_famres.rpy:87
translate simplified_chinese r1_famres_7e51760b:

    # p "W...Wait a second...!"
    p "「等、等下…！」"

# game/r1_famres.rpy:91
translate simplified_chinese r1_famres_39827c3a:

    # "Shiraishi looked at me pleadingly with eyes full of hope."
    "白石用充满了希望和强烈愿望的眼神看着[player_name!t]。"

# game/r1_famres.rpy:93
translate simplified_chinese r1_famres_5b9bcfd4:

    # p "...Okay fine."
    p "「…真拿你没办法啊」"

# game/r1_famres.rpy:97
translate simplified_chinese r1_famres_b22520bd:

    # s "Pick one, Hitona onee-chan!"
    s "「[player_name!t]姐姐，选一个喜欢的吧！」"

# game/r1_famres.rpy:109
translate simplified_chinese r1_famres_91182f92:

    # p "I’ll have the [famires_food!t] then."
    p "「那，[player_name!t]选[famires_food!t]吧」"
    "{color=[p.who_args[color]]}[p]{/color}＆{color=[s.who_args[color]]}[s]{/color}" "「「我开动了！」」"

# game/r1_famres.rpy:114
translate simplified_chinese r1_famres_1e487da6:

    # s "Hitona onee-chan, Shiraishi has something to ask."
    s "「那个，[player_name!t]姐姐。白石呢，想问点东西」"

# game/r1_famres.rpy:116
translate simplified_chinese r1_famres_90e99cb4:

    # p "What is it?"
    p "「是什么呢？」"

# game/r1_famres.rpy:118
translate simplified_chinese r1_famres_f289d762:

    # s "Do people become taller if they eat omurice?"
    s "「吃了蛋包饭的话，会不会长高呢？」"

# game/r1_famres.rpy:120
translate simplified_chinese r1_famres_d0430a98:

    # p "{i}Where did that come from??{/i}"
    p "（从哪里想到要问这个的…）"

# game/r1_famres.rpy:122
translate simplified_chinese r1_famres_9b0cc0d2:

    # p "Eh? I don’t think so."
    p "「诶？[player_name!t]觉得大概不会长高吧」"

# game/r1_famres.rpy:124
translate simplified_chinese r1_famres_9d38d428:

    # p "Usually people ask if drinking milk makes you grow taller, but omurice? That’s a new one."
    p "「想长高的话一般是喝牛奶吧？[player_name!t]是第一次听说吃了蛋包饭会长高呢！」"

# game/r1_famres.rpy:126
translate simplified_chinese r1_famres_762888f1:

    # p "You want to become taller, Shiraishi?"
    p "「小白石想要长高吗？」"

# game/r1_famres.rpy:130
translate simplified_chinese r1_famres_afec076c:

    # s "Yes! Shiraishi wants to become tall and beautiful just like Hitona onee-chan!"
    s "「嗯！白石想成为像[player_name!t]姐姐这样高挑的美少女！」"

# game/r1_famres.rpy:132
translate simplified_chinese r1_famres_112c383c:

    # p "{i}Ahh, what a precious response...{/i}"
    p "（啊，这是多么的可贵的回答…）"

# game/r1_famres.rpy:134
translate simplified_chinese r1_famres_119a1574:

    # s "Hitona onee-chan, does Hamburger Steak make you more energetic?"
    s "「话说[player_name!t]姐姐，每次吃汉堡肉都会变得更有精神吗？」"

# game/r1_famres.rpy:136
translate simplified_chinese r1_famres_53b3e489:

    # p "It’s meat... so I guess so? Why?"
    p "「这个…因为毕竟是肉吧？怎么问这个？」"

# game/r1_famres.rpy:140
translate simplified_chinese r1_famres_c4c6eba3:

    # s "Shiraishi heard of a really energetic person who loves hamburger steak!"
    s "「白石从一个非常有活力而又喜欢吃汉堡肉的人那里听说的！」"

# game/r1_famres.rpy:142
translate simplified_chinese r1_famres_270125b9:

    # s "Shiraishi thought that maybe hamburger steaks were the reason that she’s always so energetic!"
    s "「能如此精神充沛，我觉得应该是汉堡肉的功劳吧！」"

# game/r1_famres.rpy:144
translate simplified_chinese r1_famres_b155b637:

    # p "{i}What a cute thought... for some reason, it reminds me of a certain someone though.{/i}"
    p "（好可爱的想法啊…但为何会让[player_name!t]想起某个人呢）"

# game/r1_famres.rpy:146
translate simplified_chinese r1_famres_5973bd1e:

    # p "Well, I guess whenever you eat your favourite foods you become a bit happier."
    p "「嗯，只要吃最喜欢的料理就会变得很有精神呢」"

# game/r1_famres.rpy:150
translate simplified_chinese r1_famres_c650c2d1:

    # s "Does that mean that hamburger steak is Hitona onee-chan’s favourite food?"
    s "「那，[player_name!t]姐姐也是最喜欢吃汉堡肉吗？」"

# game/r1_famres.rpy:152
translate simplified_chinese r1_famres_7ba718b3:

    # p "Uhhh... no, not really..."
    p "「嗯…[player_name!t]觉得没那么喜欢吧…」"

# game/r1_famres.rpy:154
translate simplified_chinese r1_famres_584030bd:

    # s "Is that so?"
    s "「是吗？」"

# game/r1_famres.rpy:156
translate simplified_chinese r1_famres_e23771d2:

    # s "Hitona onee-chan, are you planning to play something tonight?"
    s "「那[player_name!t]姐姐，今晚要玩恐怖游戏吗？」"

# game/r1_famres.rpy:158
translate simplified_chinese r1_famres_81c9bb11:

    # p "Uhh... I don’t have any plans, so not really."
    p "「这个…倒也没有这种预定呢…」"

# game/r1_famres.rpy:160
translate simplified_chinese r1_famres_e00a9e6c:

    # s "Hitona onee-chan, do you like beef bowls?"
    s "「[player_name!t]姐姐喜欢牛肉饭吗？」"

# game/r1_famres.rpy:162
translate simplified_chinese r1_famres_a445ee32:

    # p "I guess so?"
    p "「还算喜欢吧」"

# game/r1_famres.rpy:164
translate simplified_chinese r1_famres_98a23b1f:

    # s "When Hitona onee-chan chose the beef bowl, Shiraishi thought that Hitona onee-chan was planning to play a horror game tonight!"
    s "「这样啊，白石还以为[player_name!t]姐姐选了牛肉饭就是准备今晚要玩恐怖游戏呢」"

# game/r1_famres.rpy:166
translate simplified_chinese r1_famres_acd253e0:

    # p "That’s oddly specific, why did you think so?"
    p "「太具体了吧，为什么会这么想呢？」"

# game/r1_famres.rpy:170
translate simplified_chinese r1_famres_3dd2a890:

    # s "Fortune telling!"
    s "「占卜！」"

# game/r1_famres.rpy:172
translate simplified_chinese r1_famres_56b902e5:

    # p "That’s some impressive fortune telling alright...."
    p "「好厉害的占卜啊…」"

# game/r1_famres.rpy:174
translate simplified_chinese r1_famres_6cf7da62:

    # "We somehow managed to finish eating the mountain of food that we (Shiraishi) ordered."
    "{rb}[player_name!t]和白石{/rb}{rt}主要是白石{/rt}勉强把点的大量菜品吃完了。"
    p "「我吃饱了…」"
    s "「我吃饱了！」"

# game/r1_famres.rpy:176
translate simplified_chinese r1_famres_c4bf4e03:

    # p "Ugghhh, I ate too much..."
    p "「呕…吃太多了…」"

# game/r1_famres.rpy:180
translate simplified_chinese r1_famres_445c6081:

    # s "It was yummy!"
    s "「好好吃！」"

# game/r1_famres.rpy:182
translate simplified_chinese r1_famres_06078c8a:

    # p "I can’t believe you managed to eat all of that by yourself!"
    p "「真不敢相信小白石吃了那么多！」"

# game/r1_famres.rpy:184
translate simplified_chinese r1_famres_45f30740:

    # p "You might have even been able to eat all three of them..."
    p "「那三道菜几乎都是你一个人吃完了…」"

# game/r1_famres.rpy:188
translate simplified_chinese r1_famres_9a7605ec:

    # s "No way! Shiraishi can’t eat three; it’s because of Hitona onee-chan that we finished!"
    s "「不可能！白石一个人的话不可能吃完三道菜的！是多亏了[player_name!t]姐姐才吃完的！」"

# game/r1_famres.rpy:190
translate simplified_chinese r1_famres_c50ea97f:

    # p "{i}Then don’t order three in the first place...{/i}"
    p "（这样的话就别点三道啊…）"

translate simplified_chinese strings:

    # game/r1_famres.rpy:99
    old "Which dish should I eat?"
    new "要吃哪一份？"

    # game/r1_famres.rpy:99
    old "Omurice"
    new "蛋包饭"

    # game/r1_famres.rpy:99
    old "Beef Bowl"
    new "牛肉饭"

    # game/r1_famres.rpy:99
    old "Hamburger Steak"
    new "汉堡肉"
