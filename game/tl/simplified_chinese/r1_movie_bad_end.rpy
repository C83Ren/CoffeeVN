# game/r1_movie_bad_end.rpy:4
translate simplified_chinese r1_movie_bad_end_a5b14834:

    # p "I don’t know... I’m not sure if we can meet again later, so I can’t promise."
    p "「该怎么说呢...我也不确定我们之后还能不能遇到呢，所以姐姐也无法向小白石保证呢」"

# game/r1_movie_bad_end.rpy:6
translate simplified_chinese r1_movie_bad_end_8b2ff51c:

    # p "Sorry, Shiraishi..."
    p "「对不起啊，小白石...」"

# game/r1_movie_bad_end.rpy:10
translate simplified_chinese r1_movie_bad_end_680b997e:

    # "Shiraishi smiled warmly."
    "白石成熟地微笑着。"

# game/r1_movie_bad_end.rpy:12
translate simplified_chinese r1_movie_bad_end_b29abf8f:

    # s "Hitona onee-chan is such a caring person."
    s "「人鱼姐姐好温柔！」"

# game/r1_movie_bad_end.rpy:14
translate simplified_chinese r1_movie_bad_end_ab1bb3d8:

    # s "One day, when I grow up, I want to become someone just like you."
    s "「等我长大了要成为像人鱼姐姐一样的人」"

# game/r1_movie_bad_end.rpy:18
translate simplified_chinese r1_movie_bad_end_189b4c2a:

    # "???" "Oii! Kohiiiii!"
    "？？？" "「喂！小东————！」"

# game/r1_movie_bad_end.rpy:20
translate simplified_chinese r1_movie_bad_end_a99d279d:

    # "???" "What do you want to watch?"
    "？？？" "「在想什么呢？」"

# game/r1_movie_bad_end.rpy:22
translate simplified_chinese r1_movie_bad_end_143e6df8:

    # p "Eh?"
    p "「诶？」"

# game/r1_movie_bad_end.rpy:24
translate simplified_chinese r1_movie_bad_end_5f68f506:

    # "???" "You’ve been daydreaming a lot recently, are you okay?"
    "？？？" "「你最近很容易走神呢，没事吧？」"

# game/r1_movie_bad_end.rpy:26
translate simplified_chinese r1_movie_bad_end_d0075926:

    # p "Ah yeah... I’m fine. What’s there to watch again?"
    p "「啊，嗯...没事儿。所以，现在有什么电影在放映来着？」"

# game/r1_movie_bad_end.rpy:28
translate simplified_chinese r1_movie_bad_end_4fff8749:

    # "???" "There’s..."
    "？？？" "「首先是...」"

# game/r1_movie_bad_end.rpy:30
translate simplified_chinese r1_movie_bad_end_f1d1cf11:

    # "What was that all about? Was it a dream?"
    "那个到底是什么呢？是梦吗？"

# game/r1_movie_bad_end.rpy:32
translate simplified_chinese r1_movie_bad_end_7f57b297:

    # "My memory has become fuzzy, and I can’t clearly remember it anymore."
    "记忆变得模糊了。想不起到底发生了什么事。"

# game/r1_movie_bad_end.rpy:34
translate simplified_chinese r1_movie_bad_end_3d1603d5:

    # "I guess dreams will just be dreams."
    "应该是做了一场梦吧。"
