...
def copy_presplash(directory, name, default):
    ...
    for lang in [ "en", "jp", "zh" ]:
        for ext in [ ".png", ".jpg" ]:
            fn = os.path.join(directory, name + "-" + lang + ext)

            if os.path.exists(fn):
                shutil.copy(fn, plat.path("project/app/src/main/assets/" + name + "-" + lang + ext))
                break
...
GENERATED = [
    ...
    (False, "templates/app-strings-en.xml", "project/app/src/main/res/values-en/strings.xml"),
    (False, "templates/app-strings-zh.xml", "project/app/src/main/res/values-zh/strings.xml"),
